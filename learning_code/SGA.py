...
import random
import timeit#compute the time we use
...
def getEncodedLength(delta=0.0001, boundarylist=[]):
    ...
    for i in boundarylist:
        ...
        res = fsolve(lambda x: ((upper - lower)/ delta) - 2 ** x - 1, 50)# for more details on function fslove, you can look the python code one.py
...
def getIntialPopulation(encodelength, populationSize):
    ...
    chromosomes=np.random.randint(0,2,size=(populationSize,sum(encodelength)))
    return chromosomes
...
def decodedChromosome(encodelength, chromosomes, boundarylist, delta=0.0001):
    ...
    variables = len(encodelength)#compute the dimension on the encodelength
    decodedvalues = np.zeros((populations, variables))#initialize
    for k, chromosome in enumerate(chromosomes):
        chromosome = chromosome.tolist()#变为一个list
        start = 0
...
def getFitnessValue(func, chromosomesdecoded):
    ...
    # 计算每个染色体被选择的概率
    sum_pro=np.sum(fitnessvalues)
    probability = fitnessvalues / sum_pro
    # 得到每个染色体被选中的累积概率
    cum_probability = np.cumsum(probability)#numpy.cumsum() means add all the numbers before index
...
def selectNewPopulation(chromosomes, cum_probability):
    ...
    for i, randoma in enumerate(randoms):
        ...
        newpopulation[i, :] = chromosomes[index[0][0], :]#理解这里最后的结果,主要是对于np.where的理解
...
def crossover(population, Pc=0.8):
    ...
    numbers = np.uint8(m * Pc)#convert to integer
    # 确保进行交叉的染色体个数是偶数个
    if numbers % 2 != 0:
        numbers += 1
    # 交叉后得到的新种群
    ...
    # 产生随机索引
    index = random.sample(range(m), numbers)#choose numbers from range(m)
    # 不进行交叉的染色体进行复制
    for i in range(m):
    ...
    while len(index) > 0:
        ...
        b = index.pop()
        # 随机产生一个交叉点
        crossoverPoint = random.sample(range(1, n), 1)#also you can choose not just a cross point but from one point to another
        crossoverPoint = crossoverPoint[0]#it's a list,so we should convert it into an integer
...
# 染色体变异
def mutation(population, Pm=0.01):#bacause the size of our initialize is 10,so maybe Pm choose as 1/10 will be better
    """
    ...
    #you can also use the function reshape() to do this
...
solution, value,values = main(500)
print('最优解: x1, x2')
...
#源代码种群数量过少，且出现了一些问题，导致没有实现收敛，修改之后，实际上可以看到，大概100次迭代后已经实现了收敛